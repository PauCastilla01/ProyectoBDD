    ...
    case
        when inserting then
            if :new:tamanio >10 then
                ...
                insert into archivo_programa_2(num_archivo,programa_id,tamanio)
                values (:new.num_archivo, :new.programa_id, :new.tamanio);
        ...
        when deleting then
            if :old:tamanio >10 then
                delete from archivo_programa_1 where num_archivo = :old.num_archivo;
                if sql%rowcount != 1 then
                    raise_application_error(-20040, 'No se eliminó el registro en archivo_programa_1');
                end if;
            elsif :old:tamanio <=10 then
                delete from archivo_programa_2 where num_archivo = :old.num_archivo;
                if sql%rowcount != 1 then
