...
begin
    case
        ...
        when updating then
            v_count := 0;
            -- replica local
            update pais_r1 set clave = :new.clave, nombre = :new.nombre, continente = :new.continente
                where pais_id = :new.pais_id;
            v_count := v_count + sql%rowcount;
            -- replica 2
            update pais_r2 set clave = :new.clave, nombre = :new.nombre, continente = :new.continente
                where pais_id = :new.pais_id;
            v_count := v_count + sql%rowcount;
            -- replica 3
            update pais_r3 set clave = :new.clave, nombre = :new.nombre, continente = :new.continente
                where pais_id = :new.pais_id;
            v_count := v_count + sql%rowcount;
            -- replica 4
            update pais_r4 set clave = :new.clave, nombre = :new.nombre, continente = :new.continente
                where pais_id = :new.pais_id;
            v_count := v_count + sql%rowcount;
            if v_count <> 4 then
                raise_application_error(-20040,
