...
--@Fecha creación:
--@Descripción: trigger tipo_cuenta
...
begin
    case
        ...
        when updating then
            v_count := 0;
            -- replica local
            update tipo_cuenta_r1 set clave = :new.clave, descripcion = :new.descripcion, costo_mensual = :new.costo_mensual
                where tipo_cuenta_id = :new.tipo_cuenta_id;
            v_count := v_count + sql%rowcount;
            -- replica 2
            update tipo_cuenta_r2 set clave = :new.clave, descripcion = :new.descripcion, costo_mensual = :new.costo_mensual
                where tipo_cuenta_id = :new.tipo_cuenta_id;
            v_count := v_count + sql%rowcount;
            -- replica 3
            update tipo_cuenta_r3 set clave = :new.clave, descripcion = :new.descripcion, costo_mensual = :new.costo_mensual
                where tipo_cuenta_id = :new.tipo_cuenta_id;
            v_count := v_count + sql%rowcount;
            -- replica 4
            update tipo_cuenta_r4 set clave = :new.clave, descripcion = :new.descripcion, costo_mensual = :new.costo_mensual
                where tipo_cuenta_id = :new.tipo_cuenta_id;
            v_count := v_count + sql%rowcount;
            if v_count <> 4 then
                raise_application_error(-20040,
